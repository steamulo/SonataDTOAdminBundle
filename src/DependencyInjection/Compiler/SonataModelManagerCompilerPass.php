    ...
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ($container->findTaggedServiceIds('sonata.admin.dto_repository') as $serviceId => $attributes) {
            ...
            if (!isset($attribute['class'])) {
                throw new \RuntimeException(sprintf('%s: Missing "class" attribute for tag "sonata.admin.dto_repository"', $serviceId));
            }
        ...
        foreach ($container->findTaggedServiceIds('sonata.admin.dto_identifier_normalizer') as $serviceId => $attributes) {
            $attribute = reset($attributes);
            if (!isset($attribute['class'])) {
                throw new \RuntimeException(sprintf('%s: Missing "class" attribute for tag "sonata.admin.dto_identifier_normalizer"', $serviceId));
            }

            $modelManagerDefinition->addMethodCall('addIdentifierNormalizer', [$attribute['class'], new Reference($serviceId)]);
        }

        foreach ($container->findTaggedServiceIds('sonata.admin.dto_identifier_denormalizer') as $serviceId => $attributes) {
            $attribute = reset($attributes);
            if (!isset($attribute['class'])) {
                throw new \RuntimeException(sprintf('%s: Missing "class" attribute for tag "sonata.admin.dto_identifier_denormalizer"', $serviceId));
            }

            $modelManagerDefinition->addMethodCall('addIdentifierDenormalizer', [$attribute['class'], new Reference($serviceId)]);
        }
    }
}
